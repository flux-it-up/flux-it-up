<div>
    <x-button :text="__('Create Repair Request')" wire:click="$toggle('modal')" sm />

    <x-modal :title="__('Create Repair Request')" wire x-on:open="setTimeout(() => $refs.name.focus(), 250)">
        <form id="repair-request-create" wire:submit="save" class="space-y-4">
            <div>
            ...
            <div>
                <x-select.styled label="{{ __('Services') }} *" placeholder="Choose service..." wire:model.live="selectedServices" search searchable multiple :options="$services" select="label:name|value:id" />
            </div>
            <div>
            ...
            <div>
                <x-select.styled label="{{ __('Assign Technician') }}" placeholder="Choose technician..." wire:model="repair.technician_id" searchable :options="$technicians" select="label:name|value:id" />
            </div>
            <div>
        ...
        <x-slot:footer>
            <x-button type="submit" form="repair-request-create">
                @lang('Save')
            </x-button>
        </x-slot:footer>
    </x-modal>
