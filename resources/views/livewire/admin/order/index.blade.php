<div>
    <x-card>
        <div class="mb-2 mt-4">
            <livewire:admin.order.create @created="$refresh" />
        </div>
        ...
        <x-table :$headers :$sort :rows="$this->rows" paginate simple-pagination filter loading :quantity="[5,25,50,100]">
            ...
            @interact('column_action', $row)
                <div class="flex gap-1">
                    ...
                    {{-- <livewire:admin.order.delete :product="$row" :key="uniqid('', true)" @deleted="$refresh" /> --}}
                </div>
            @endinteract
        </x-table>
    </x-card>

    <livewire:admin.order.update @updated="$refresh" />
</div>
