<div>
    <x-card>
        <div class="mb-2 mt-4">
            <livewire:admin.pricing-tier.create @created="$refresh" />
        </div>
        ...
        <x-table :$headers :$sort :rows="$this->rows" paginate simple-pagination filter loading :quantity="[5,25,50,100]">
            ...
            <div class="flex gap-1">
                ...
                {{-- <livewire:admin.pricing-tier.delete :service="$row" :key="uniqid('', true)" @deleted="$refresh" /> --}}
            </div>
            @endinteract
        </x-table>
    </x-card>

    <livewire:admin.pricing-tier.update @updated="$refresh" />
</div>
